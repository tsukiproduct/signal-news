...
def render(lang, news, frame, select, card, categories, path, t):
    ...
    title = t(lang,'AIニュースを、分かりやすく。','AI news, clearly explained.')
...
<div class="intro"><h1>{t(lang,'AIニュース','AI news')}</h1><p>{t(lang,'何が変わったかを短く。気になるニュースは、出典まで。','A concise view of what changed. Follow the source when you want more.')}</p></div>
...
<div class="toolbar"><label class="search">{t(lang,'ニュースを検索','Search news')}<input id="news-search" type="search" placeholder="{t(lang,'ツール名や気になるキーワード','A tool or topic you follow')}"></label><label>{t(lang,'表示する記事','Show')}<select id="news-mode"><option value="all">{t(lang,'すべて','All stories')}</option><option value="saved">{t(lang,'あとで読む','Saved')}</option><option value="unread">{t(lang,'未読','Unread')}</option></select></label><label>{t(lang,'並び順','Sort')}<select id="news-sort"><option value="useful">{t(lang,'選定順','Editorial order')}</option><option value="latest">{t(lang,'新しい順','Newest')}</option></select></label><details class="display-settings"><summary>{t(lang,'文字サイズ・画面の明るさ','Text size & appearance')}</summary><button type="button" data-display="large" aria-pressed="false">{t(lang,'文字を大きく','Larger text')}</button><button type="button" data-display="dark" aria-pressed="false">{t(lang,'暗い表示','Dark theme')}</button></details></div>
...
<div class="layout"><section aria-label="{t(lang,'ニュース一覧','News feed')}"><p id="result-count" class="count" role="status">{len(items)} {t(lang,'件の記事','stories')}</p><p id="empty" class="empty" {'hidden' if items else ''}>{t(lang,'該当する記事がありません。条件を変えてお試しください。','No matching stories. Try changing the filters.')}</p><div id="news-list">{cards}</div>{support(lang,True)}</section>
<aside class="rail"><section class="rail-block"><span class="kicker">{t(lang,'読む、その先へ','READ FURTHER')}</span><h2>{t(lang,'選ぶ前に知っておきたいこと','Before you choose')}</h2><ul class="rail-list">{guide_links}</ul></section>{support(lang)}
