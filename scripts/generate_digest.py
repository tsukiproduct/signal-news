...
読者が5分以内で把握できるよう、本文全体は日本語で1600字以内を目安にする。
各ニュースは「何が変わったか」「誰に関係するか」「出典で確認する条件」を短く示す。
価格・提供地域・商用利用・利用上限・新機能など、実際の利用判断に影響する変更を優先する。
入力に根拠のないトピックは省略する。プロンプトやバイラル枠を埋めるための創作はしない。
出力はJSONのみ。前置き不要。"""
